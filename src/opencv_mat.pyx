...
cdef Mat np2Mat2D_S32(np.ndarray ary):
    ...
    memcpy(m.data, np_buff.data, r*c*sizeof(int)) # 4 is the size of 
    return m
...
cdef object Mat2np(Mat m):
    ...
    cdef size_t length = m.rows*m.cols*m.elemSize()#m.channels()*sizeof(CV_8UC3)
    # Fill buffer
    PyBuffer_FillInfo(&buf_info, NULL, m.data, length, 1, PyBUF_FULL_RO)
    ...
    if m.depth() == CV_32F :
        ...
        ary = np.ndarray(shape=shape_array, buffer=Pydata, order='c', dtype=np.int32)
    else :
    #8-bit image
